...
def load_dataset_with_labels(max_length, max_n_examples, tokenize=False, max_vocab_size=2048, data_dir=''):
    ...
    with open(data_dir, 'r') as f:
        for line in f:
            ...
            sequences.append(sequence + (("P",)*(max_length-len(sequence))))
            labels.append(label)
            
            if len(sequences) == max_n_examples:
                finished = True
                break

    if not finished:
        print("Warning: Reached end of file before reaching max_n_examples.")

    np.random.shuffle(list(zip(sequences, labels)))  # Shuffle both sequences and labels together

    sequences, labels = zip(*list(zip(sequences, labels)))  # Unzip back to sequences and labels

    import collections
    counts = collections.Counter(char for seq in sequences for char in seq)
    ...
    for seq in sequences:
        ...
        filtered_sequences.append(tuple(filtered_seq))

    print("Loaded {} sequences and {} labels.".format(len(sequences), len(labels)))
    return filtered_sequences, charmap, inv_charmap, labels