...
from fast_MLT_MLPClassifier import *
...
class WGAN_LangGP():
    def __init__(self, batch_size=64, lr=0.0001, num_epochs=150, seq_len = 156, data_dir='./data/multi_task_train.fasta', \
        run_name='test', hidden=512, d_steps = 10, max_examples=2600):
    ...
    def train_model(self, load_dir):
        ...
        for epoch in range(1, self.n_epochs+1):   
            ...
            with open(self.sample_dir + "sampled_{}_preds.txt".format(epoch), 'w+') as f:
                for j, seq in enumerate(sampled_seqs):
                    ...
                    if condition_met:
                        ...
                        f.write(seq + '\t' + str(round(preds1[j].item(),4)) + '\t' + str(round(preds2[j].item(),4)) + '\n')
                        if preds1[j] >= 0.8 and preds2[j] >= 0.8:
                            tmp = tmp + 1
                    else:
                        f.write(seq + '\t' + str(0.0) + '\t' + str(0.0) + '\n')
                        preds1[j] = 0.0
                        preds2[j] = 0.0
            ...
            for idx in range(n_batches):
                ...
                p1 = torch.round(preds['0'] * 10**4) / 10**4
                p2 = torch.round(preds['1'] * 10**4) / 10**4
                ...
                cl_err =  0.7 * (torch.mean(torch.log(p1*p2+ 1e-6)))
                g_err = g_err - cl_err
...
def main():
    ...
    parser.add_argument("--run_name", default= "mlt_cdgan_amp=0.7_constrains_multi", help="Name for output files")
